...
if(isset($_POST['submit'] )) //if submit btn is pressed
{
    ...
    $v_code = mysqli_real_escape_string($db,bin2hex(random_bytes(16)));
    // $code = "inactive";
    ...
    if(mysqli_num_rows(mysqli_query($db, "SELECT email FROM users where email = '{$email}'")) > 0 ){
        ...
        $msg = "<div class='alert alert-danger'>Password length must be 7 digits</div>";
    }elseif(strlen($_POST['phone']) < 10){
        $msg = "<div class='alert alert-danger'>Invalid phone number</div>";
    // }elseif(mysqli_num_rows(mysqli_query($db, "SELECT email FROM users where username='{$username}'")) > 0 ){
    //     $msg = "<div class='alert alert-danger'>Username has been already exist</div>";
    }elseif($_POST['password'] === $_POST['cpassword']){
        $sql = "INSERT INTO users(username,f_name,l_name,email,phone,password,address,verification_code,is_verified) 
        VALUES('{$uname}','{$fname}','{$lname}','{$email}','{$phone}','{$pass}','{$address}','{$v_code}','0')";
    ...
    if($result){
        ...
        <a href="http://localhost/tayeb/Foodpicky/verify.php?email='.$email.'">Verify</a>';
    ...
    $msg = "<div class='alert alert-danger'>Password & confirm password do not match!</div>";
}
                                 ...
                                 <div class="row">
								  <div class="form-group col-sm-12">
                                    ...
                                    <div class="form-group col-sm-6">
                                       ...
                                       <input class="form-control" type="text" name="phone" id="example-tel-input-3" placeholder="Phone" required> <small class="form-text text-muted">We"ll never share your email with anyone else.</small> 
                                    </div>
                                    <div class="form-group col-sm-6">
